...
func TestSM4StandardExample(t *testing.T) {
	temp := []byte{0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef, 0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10}
	cipher, err := NewCipher(temp)
	if err != nil {
		t.Error(err)
	}
	for i := 0; i < 1000000; i++ {
	...
	if bytes.Compare(temp, []byte{0x59, 0x52, 0x98, 0xc7, 0xc6, 0xfd, 0x27, 0x1f, 0x04, 0x02, 0xf8, 0x04, 0xc3, 0x3d, 0x3f, 0x66}) != 0 {
		t.Error("failed")
	}
}
